...
![](https://i.imgur.com/ZuaIh2V.png)
	...
	交换机更具层次型结构划分：
	接入层交换机。接入层使工作站连接的入口，实现用户的网络访问控制，这一层的交换机应该以低成本提供高密度的接入网络。
...
##### 交换机的基本配置
	...
	#配置接口GE1/0/0和GE1/0/2的端口隔离功能，实现两个接口之间的二层数据隔离，三层数据互通
	<Switch1>system-view
...
##### 接入层交换机的VLAN划分
	...
	[SwitchA-GigabitEthernet]port link-type trunk    //配置上联接口类型
	[SwitchA-GigabitEthernet]port trunk allow-pass vlan2   //通过VLAN2
...
![](https://i.imgur.com/m4pnGHb.png)
	...
	[Server-ui-vty0-4]aythentication-mode aaa
